...
public class CodeFactory {
	...
	public void creatCode(CaptnProtoSchema schema, CodeProperties properties) throws IOException {
		...
		for (CaptnProtoEnum captainEnum : schema.getEnums()) {
			...
			enumFile.addUnion(context.newEnumGenerator(context, captainEnum));
			enumFile.generate();
		}
		for (CaptnProtoContainer container : schema.getStructs()) {
			switch (container.getType()) {
			case STRUCT:
				StructGenerator structGenerator = context.newStructGenerator(context, container);
	...
	private void handleContainer(ContainerGenerator parent, CaptnProtoContainer captainContainer,
			CodeContext context) throws IOException {
		for (CaptnProtoEnum captainEnum : captainContainer.getEnums()) {
			...
			addEnum(parent, captainEnum, context);
		}
	...
	private void addEnum(ContainerGenerator builder, CaptnProtoEnum captainEnum, CodeContext context) throws IOException {
		builder.addEnum(captainEnum);
	}
